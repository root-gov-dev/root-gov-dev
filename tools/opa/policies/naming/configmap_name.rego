package naming.configmap

import data.common

# File purpose: Naming and required labels for ConfigMap resources.
...
violation[msg] {
  input.kind == "ConfigMap"
  not allow
  msg := sprintf("configmap '%s' violates naming/labels policy", [input.metadata.name])
}